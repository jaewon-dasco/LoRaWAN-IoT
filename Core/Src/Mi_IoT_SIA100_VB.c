...
const IoTParameter_t MiIoT_DefaultParameter = {
	.Information.ProductCode = IoTProductType_SIA100_SD,
...
oResult_t MiIoT_IsValidParameter(IoTParameter_t *pParameter)
{
	...
	if(pParameter == NULL || pParameter->Information.ProductCode != IoTProductType_SIA100_SD){
		return RESULT_NULL;
	}
...
{
	if(pPayload == NULL || ProductCode != IoTProductType_SIA100_SD){
		return RESULT_NULL;
	}
