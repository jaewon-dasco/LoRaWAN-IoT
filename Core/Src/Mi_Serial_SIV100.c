...
{
	if(pPacket == NULL || pPacket->TypeOfData != IoTDataType_Analog){
		return;
	}

	IoTDataAnalog_t *pData = (IoTDataAnalog_t *)pPacket->Frame;
	int i;
	...
	oSerial_Printf(&MiSerial_Handler, "\"Analog\" : [\r\n");

	for(i=0; i<MIIOT_VW_CH_MAX_COUNT*2; i++){
		oSerial_Printf(&MiSerial_Handler, "{");
		oSerial_Printf(&MiSerial_Handler, "\"Type\" : \"%d\", ", (int)pData->Channel[i].Type);

		switch(pData->Channel[i].Type)
		{
			case IoTSensorType_Thermistor:
				oSerial_Printf(&MiSerial_Handler, "\"Analog\" : \"%.1f\"", (float)MIIOT_DATA_DECODE_TEMP(pData->Channel[i].Analog));
				break;
			case IoTSensorType_VibratingWire:
				oSerial_Printf(&MiSerial_Handler, "\"Analog\" : \"%.2f\"", (float)MIIOT_DATA_DECODE_FREQUENCY(pData->Channel[i].Analog));
				break;
			default:
				oSerial_Printf(&MiSerial_Handler, "\"Analog\" : \"0\"");
				break;
		}

		oSerial_Printf(&MiSerial_Handler, "}");

		if(i<MIIOT_VW_CH_MAX_COUNT*2-1){
			oSerial_Printf(&MiSerial_Handler, ",");
		}
