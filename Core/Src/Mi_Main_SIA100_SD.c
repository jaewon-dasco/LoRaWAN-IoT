...
oIO_t DO_BATTERY_CHECK	= {DO_BATTERY_CHECK_GPIO_Port,	DO_BATTERY_CHECK_Pin,		IO_LOW};
...
oResult_t MiMain_GPIOControl()
{
	...
	IO_WRITE(DO_BATTERY_CHECK, GPIOs.DO.BatteryCheck);
...
void MiMain_GPIODeInit(void)
{
	...
	cfg.Pin = DO_BATTERY_CHECK.Pin;
	HAL_GPIO_Init(DO_BATTERY_CHECK.Port, &cfg);
	cfg.Pin = DO_LED_OPERATING.Pin;
	HAL_GPIO_Init(DO_LED_OPERATING.Port, &cfg);

	cfg.Pin = DI_MEMS_EVENT1.Pin;
	HAL_GPIO_Init(DI_MEMS_EVENT1.Port, &cfg);
	cfg.Pin = DI_MEMS_EVENT2.Pin;
	HAL_GPIO_Init(DI_MEMS_EVENT2.Port, &cfg);
