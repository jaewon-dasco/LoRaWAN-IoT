...
IoT_MailboxItem_t* MiIoT_MailBox_AddItem(IoT_Mailbox_t *pMailBox, IoT_MailboxItem_t *pItem)
{
	...
	pMailBox->LastIndex %= MIIOT_MAILBOX_MAXCOUNT;
...
{
	if(pMailBox == NULL) return NULL;
	return &pMailBox->Item[pMailBox->LastIndex % MIIOT_MAILBOX_MAXCOUNT];
}
...
IoT_MailboxItem_t* MiIoT_MailBox_GetItem(IoT_Mailbox_t* pMailBox)
{
	...
	pMailBox->FirstIndex %= MIIOT_MAILBOX_MAXCOUNT;
	...
	if(MiIoT_GPIODeInitCallback && MiIoT_GPIOInitCallback){
		...
		switch(MiIoT_SleepStep)
		{
			...
			case 1:
				...
				break;
			case 2:
				Native_SleepMode(MiIoT_Parameter.Operating.OperatingMode == IoTOperatingMode_Stop ? 0 : SECOND_TO_MS(10)); //Original code
